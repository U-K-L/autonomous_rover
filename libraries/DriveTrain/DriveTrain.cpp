...
void DriveTrain::moveTo(int servoId, float destination) {
...
void DriveTrain::spinAt(int servoId, float velocity) {
...
int DriveTrain::positionRead(int id) {
	return LobotSerialServoReadPosition(Serial1, id);
}

int DriveTrain::getVoltage(int id) {
...
void DriveTrain::LobotSerialServoMove(HardwareSerial &SerialX, uint8_t id, int16_t position, uint16_t time) {
	byte buf[10];
	if(position < 0)
	position = 0;
	if(position > 1000)
...
void DriveTrain::LobotSerialServoStopMove(HardwareSerial &SerialX, uint8_t id) {
	byte buf[6];
	buf[0] = buf[1] = LOBOT_SERVO_FRAME_HEADER;
	buf[2] = id;
	buf[3] = 3;
...
void DriveTrain::LobotSerialServoSetID(HardwareSerial &SerialX, uint8_t oldID, uint8_t newID) {
	byte buf[7];
	buf[0] = buf[1] = LOBOT_SERVO_FRAME_HEADER;
	buf[2] = oldID;
	buf[3] = 4;
...
void DriveTrain::LobotSerialServoSetMode(HardwareSerial &SerialX, uint8_t id, uint8_t Mode, int16_t Speed) {
	byte buf[10];
...
void DriveTrain::LobotSerialServoLoad(HardwareSerial &SerialX, uint8_t id) {
	byte buf[7];
	buf[0] = buf[1] = LOBOT_SERVO_FRAME_HEADER;
	buf[2] = id;
	buf[3] = 4;
...
void DriveTrain::LobotSerialServoUnload(HardwareSerial &SerialX, uint8_t id) {
	byte buf[7];
	buf[0] = buf[1] = LOBOT_SERVO_FRAME_HEADER;
	buf[2] = id;
	buf[3] = 4;
...
int DriveTrain::LobotSerialServoReceiveHandle(HardwareSerial &SerialX, byte *ret) {
...
int DriveTrain::LobotSerialServoReadPosition(HardwareSerial &SerialX, uint8_t id) {
	int count = 10000;
	int ret;
	byte buf[6];
...
int DriveTrain::LobotSerialServoReadVin(HardwareSerial &SerialX, uint8_t id) {
	int count = 10000;
	int ret;
	byte buf[6];
